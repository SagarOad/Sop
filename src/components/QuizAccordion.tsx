        ...
        {Data.map((item, i) => (
          <div className="bg-[#E7E7E7] dark:bg-boxdark border dark:border-strokedark">
            <div className="title flex items-center justify-between px-3 md:px-8 py-4">
              <h1
                onClick={() => toggle(i)}
                className="cursor-pointer rounded-md bg-[#86ACF5] p-3 text-[16px] md:text-[21px] text-white"
              >
                {item.header}
              </h1>
              <h2 className="text-[16px] md:text-[21px] text-[#AEAEAE]">{item.date}</h2>
            </div>
            <div className={selected == i ? 'content show' : 'content'}>
              <div className="flex justify-between bg-[#292A33] p-3 md:p-8">
                <div className="text-[14px] md:text-[18px] text-[#AEAEAE]">
